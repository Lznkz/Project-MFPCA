...
def build_basis_only(cycle, order, min_points_ratio=2.0):
    n = len(cycle)
    n_knots = min(20, max(4, n // 4))
    n_basis = n_knots + order
    ...
    # Lưu ý: BSplineBasis là class/hàm sinh spline tùy chỉnh của hệ thống bạn
    local_basis = BSplineBasis(
        domain_range=(cycle[0], cycle[-1]), n_basis=n_basis, order=order
    )

    B = local_basis(cycle)[:, :, 0].T
    BtB = B.T @ B

    try:
        L = np.linalg.cholesky(BtB)   # BtB = L Lt, dùng để giải nhanh, ổn định
    except np.linalg.LinAlgError as e:
...
def compute_hat_matrix_cholesky(B, L, cycle_eval, basis_func):
    B_eval = basis_func(cycle_eval)[:, :, 0].T

    # 2. Giải hệ phương trình tuyến tính qua Cholesky để tìm (B^T B)^-1 @ B.T
    Z = np.linalg.solve(L, B.T)          # L @ Z = B.T
    X = np.linalg.solve(L.T, Z)          # L.T @ X = Z

    # 3. Tính Hat Matrix cuối cùng
    H = B_eval @ X

    return H

def apply_batch_smoothing(H, Y_batch):
    return H @ Y_batch


def run_smoothing_for_candidate(candidate_df, cycle_validate, sensor_cols, num_eval_points=20):
    cycle = np.arange(1, cycle_validate + 1)
    cycle_eval = np.linspace(1, cycle_validate, num_eval_points)

    # Khởi tạo Basis và Cholesky
    basis_info = build_basis_only(cycle=cycle, order=3)
    if "error" in basis_info:
        raise ValueError(f"Lỗi khởi tạo Basis: {basis_info['error']}")

    # Tính Hat Matrix
    H_matrix = compute_hat_matrix_cholesky(
        B=basis_info["B"],
        L=basis_info["L"],
        cycle_eval=cycle_eval,
        basis_func=basis_info["basis"]
    )
    unique_units = candidate_df['unit_number'].unique()

    Y_list = []
    for u in unique_units:
        unit_data = candidate_df[candidate_df['unit_number'] == u][sensor_cols].values
        Y_list.append(unit_data)
    Y_batch = np.hstack(Y_list)
    Y_smoothed_batch = apply_batch_smoothing(H_matrix, Y_batch)
    smoothed_units = np.hsplit(Y_smoothed_batch, len(unique_units))

    results = {u: smoothed_units[i] for i, u in enumerate(unique_units)}
