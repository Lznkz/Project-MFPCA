...
[[id:643731f9-ae57-426d-895a-a82fae217289][2. B-Spline Smoothing (From Discrete to Functional Data)]].
...
*Generalized-Eigenvalue Parameterization* (Demmler-Reinsch) was applied to
...
- From [[*Extras*][this paper]], it has been established that each engine operates with
...
- Before computing distance, each sensor's smoothed values are normalized by
  ...
  sensor at time t, $\bar{X}_{j}^{*}(t)$ is the mean of the jth sensor, and J is the
  number of sensors
...
The current pipeline treats each sensor curve as a purely statistical object —
smoothed via B-splines without encoding any physical knowledge of the underlying
degradation process. A prior attempt was made to apply Multi-Group SINDy
(MGSINDy) — a sparse equation-discovery algorithm — to identify governing
degradation dynamics directly from the C-MAPSS sensor data for RUL prediction.
However, this approach did not yield satisfactory results, likely due to the
high noise level and the complexity of multivariate degradation interactions in
this dataset.

This experience suggests that the challenge may lie less in /discovering/
degradation equations from data and more in /enforcing/ known physical structure
during the learning process. Since the C-MAPSS dataset is generated from a
thermodynamic simulation with known governing equations, Physics-Informed Neural
Networks (PINNs) offer a promising alternative: rather than attempting to
recover equations from noisy observations (as SINDy does), PINNs embed physical
constraints (e.g., monotonic degradation trends, conservation laws) directly
into the curve-fitting step via the loss function. This would preserve the
interpretability that motivates this work while potentially capturing
degradation dynamics that B-splines cannot represent. A combined SINDy → PINN
pipeline — where partially discovered structure informs the physics loss — also
remains an open direction worth exploring.
